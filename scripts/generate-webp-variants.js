...
async function toWebpVariant(file, suffix, quality){
  const out = file.replace(/\.[a-z0-9]+$/i, `-${suffix}.webp`);
  try{
    ...
    console.warn('Variant convert failed', file, err.message);
    return null;
  }
}
...
async function main(){
  ...
  const backupDir = path.join(process.cwd(),'backup',`webp-variants-${now}`);
  await backupFiles(files, backupDir);
  console.log(`Backed up ${files.length} files to ${backupDir}`);

  const variants = [ {suffix:'low', quality:60}, {suffix:'med', quality:80}, {suffix:'high', quality:90} ];
  let created = 0;
  for(const f of files){
    for(const v of variants){
      const out = await toWebpVariant(f, v.suffix, v.quality);
      if(out) { created++; console.log('Created', out) }
    }
  }
  console.log(`WebP variants complete. Created ${created} files.`);
}
